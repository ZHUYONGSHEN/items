...
//    倒计时
    ...
    function registPanoEvent(pano) {
        ...
        touch.on("#" + canvas.id, 'touchend', function (e) {
            if (dis < 1) {
                if (!pano.bIsChanged) {
                	if(typeof (pano.clearScreen) == "function"){
                		pano.clearScreen();
                	}
    ...
    Panorama.prototype.autoPlay = function (pano){
    	if(pano.autoPlayState == true){
        	...
        	(function autoTime (){
            	...
            	//当前场景所在的组索引
            	console.log($('.switch_bar .scene_group_desc.select').index(),curSceneGroupIndex);
        ...
        for (var i = 0; i < groups.length; i++) {
            ...
            if (groups[i].title!="") {
            	...
            	if(groups.length - 1 == 0 && groups[0].title === "未分组" ){
            		$(group_bar).css('visibility', 'hidden');
            	}
        ...
        function reload() {
            if (pano.project.enable_autorotation === true &&
                ...
                if(starts-euler.y>2*Math.PI){
                    ...
                    /*if(pano.autoPlayState==true){
                    	pano.autoPlay(pano);
                    	console.log(pano.autoAddNumber);
                    }*/
                }
