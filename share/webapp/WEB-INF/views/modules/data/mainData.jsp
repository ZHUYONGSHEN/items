			...
			<div class="data_detail_keydata">
				...
				<div class="datacommontable data_detail_keytable" style="border-bottom:0px;">
					<li class="title clear">
						<p style="width:15%">统计日期</p>
						<p style="width:14%">渠道</p>
						<p style="width:14%">访问用户数</p>
						<p style="width:14%">访问次数</p>
						<p style="width:14%">拨打电话数</p>
						<p style="width:15%">VR看房平均时长(秒)</p>
						<p style="width:14%" style="padding:10px 0px 10px 55px">分享次数</p>
					</li>
				</div>
				<div class="datacommontable data_detail_keytable data_detail_keytable_scroll">
					<c:forEach var="item" items="${list}">
						<li class="clear">
							<p style="width:15%"><fmt:formatDate value="${item.day}" pattern="yyyy-MM-dd"/></p>
							<p style="width:14%">${item.channel_name }</p>
							<p style="width:14%">${item.wx_access_user_num}</p>
							<p style="width:14%">${item.access_num}</p>
							<p style="width:14%">${item.sy_lxdh}</p>
							<p style="width:15%">${item.vr_average_time}</p>
							<p style="width:14%">${item.share_num}</p>
						</li>
					</c:forEach>
				</div>
